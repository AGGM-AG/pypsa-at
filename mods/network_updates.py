...
from mods.clustering import (
    VALID_CONFIGURATIONS,
    map_at_nuts3_to_nuts2,
    map_de_nuts1_to_de5,
)
...
def override_gas_storage_capacities(n: pypsa.Network, snakemake: Snakemake) -> None:
    ...
    file_path = Path(__file__).parents[1] / "data" / "pypsa-at" / file_name
    ...
    if clustering.startswith("AT10"):
        storage = storage.groupby(storage.index.map(map_at_nuts3_to_nuts2)).sum()
    if clustering.endswith("DE5"):
        storage = storage.groupby(storage.index.map(map_de_nuts1_to_de5)).sum()
