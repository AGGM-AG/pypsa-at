"""Tests to compare the predecessor with esmtools evaluation results."""
...
class Constants:
    ...
    FILE_NAMES_BARCHARTS = [
        ...
        # missing in old predecessor implementation
...
def _barchart_file_names():
    ...
    for location, file_name_template in product(
        ...
        if file_name in (
            ...
            reason = (
                "Heat capacities in the old predecessor have a bug. "
            ...
            reason = (
                "The AC split has a Bug in the old predecessor "
            ...
            reason = (
                "Final Demand in old predecessor shows unreasonably "
            ...
            reason = (
                "The predecessor function 'get_district_heat_balances_data' "
    ...
    for location, file_name_template in product(
        ...
        if file_name == "final_demand_sec_EU":
            ...
            reason = (
                ...
                "are counted as production in the predecessor implementation."
...
def _time_series_file_names():
    ...
    for location, year, file_name in product(
        ...
        if file_name == "elec_prod_dem_time_2040_LV":
            reason = (
                "The predecessor wrongly counts the 'MISS.*' Link at port 1 as Base Load."
            )
            ...
            reason = (
                "The predecessor aggregates nodes to countries before "
            ...
            reason = (
                "The predecessor either misses the technologies ["
            ...
            reason = (
                "Small differences in the predecessor implementation for "
...
def test_compare_barchart_data(file_name: str):
    ...
    # The old predecessor has Electricity (OCGT) instead of the usual and
...
def tests_compare_timeseries_data(file_name):
    ...
    # because that is what the old predecessor does before JSON export.
    # (we also need to do the same for the old values, because the
    # predecessor does not do this consistently for all time series, e.g.
    ...
    # The predecessor rounds before clipping, while esmtools only clips
