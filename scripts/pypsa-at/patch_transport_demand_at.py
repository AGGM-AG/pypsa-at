...
def transform_nea_to_km(
    ...
    nea_filtered = nea_filtered.merge(efficiency, on=["NUTS-2 Code", "technology"])
    nea_filtered["value"] *= nea_filtered["eff"]
    return nea_filtered.groupby(["NUTS-2 Code"], as_index=False)["value"].sum()


def filter_nea(nea_at: pd.DataFrame, base_year: int) -> pd.DataFrame:
    """
    Filter Nutzenergieanalyse (nea) data to given year and road transport data
    ...
    :
        Filtered nea data grouped by NUTS2 region and energy carrier
    """
    year = nea_at.loc[nea_at["year"] <= base_year, "year"].max()
    nea_filtered = nea_at[
        (nea_at["year"] == year)
    ...
    return nea_filtered[nea_filtered["value"] != 0]
...
def main(snakemake: Snakemake) -> None:
    ...
    nea_filtered = filter_nea(nea_at, snakemake.params.planning_horizons[0])
