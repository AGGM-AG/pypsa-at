...
class Regex:
    ...
    # startswith 2 capital letters, followed by up to 3 digits
    # or A-G for german NUTS1 regions.
    # any number of digits
    region: re.Pattern = re.compile(r"^[A-Z]{2}[\d,A-G]{0,3}\s")
...
ALIAS_COUNTRY: frozendict = frozendict(
    {
        ...
        "SI": "Slovenia",
        "SK": "Slovakia",
        "XK": "User Assigned (todo: find out)",
    }
)
...
try:
    package_version = importlib.metadata.version("pypsa-at")
except PackageNotFoundError:
    package_version = "pypsa-at not installed."
...
RUN_META_DATA = {
    "esmtools": f"{package_version} ({git_hash})",
