...
def eval_electricity_balance_ts(
    ...
    metric.cfg.plotly.cutoff = 0.04999999  # None in the original predecessor
    # the original predecessor discards values smaller than 0.05 from
...
def _calculate_inflexible_demand(statistics: pd.DataFrame) -> pd.DataFrame:
    ...
    #  must be prevented. However, this is the case in the old predecessor
    #  too.
    inflexible_carrier = [
        carrier
        for carrier, group in mapping.items()
